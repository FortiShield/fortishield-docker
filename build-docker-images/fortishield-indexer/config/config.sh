...
export USER=${NAME}
export GROUP=${NAME}
export VERSION=${FORTISHIELD_VERSION}-${FORTISHIELD_TAG_REVISION}
...
export INDEXER_FILE=fortishield-indexer-base.tar.xz
export BASE_FILE=fortishield-indexer-base-${VERSION}-linux-x64.tar.xz
...
## Variables
CERT_TOOL=fortishield-certs-tool.sh
PASSWORD_TOOL=fortishield-passwords-tool.sh
PACKAGES_URL=https://packages.fortishield.com/4.8/
PACKAGES_DEV_URL=https://packages-dev.fortishield.com/4.8/
...
# Move configuration files for fortishield-indexer
mv -f ${BASE_DIR}/etc/init.d/${NAME} ${TARGET_DIR}/etc/init.d/${NAME}
mv -f ${BASE_DIR}/etc/fortishield-indexer/* ${TARGET_DIR}${CONFIG_DIR}
...
# Copy FortiShield indexer's certificates
cp -pr /fortishield-certificates/demo.indexer.pem ${TARGET_DIR}${CONFIG_DIR}/certs/indexer.pem
cp -pr /fortishield-certificates/demo.indexer-key.pem ${TARGET_DIR}${CONFIG_DIR}/certs/indexer-key.pem
cp -pr /fortishield-certificates/root-ca.key ${TARGET_DIR}${CONFIG_DIR}/certs/root-ca.key
cp -pr /fortishield-certificates/root-ca.pem ${TARGET_DIR}${CONFIG_DIR}/certs/root-ca.pem
cp -pr /fortishield-certificates/admin.pem ${TARGET_DIR}${CONFIG_DIR}/certs/admin.pem
cp -pr /fortishield-certificates/admin-key.pem ${TARGET_DIR}${CONFIG_DIR}/certs/admin-key.pem
...
sed -i 's/-Djava.security.policy=file:\/\/\/etc\/fortishield-indexer\/opensearch-performance-analyzer\/opensearch_security.policy/-Djava.security.policy=file:\/\/\/usr\/share\/fortishield-indexer\/opensearch-performance-analyzer\/opensearch_security.policy/g' ${TARGET_DIR}${CONFIG_DIR}/jvm.options
