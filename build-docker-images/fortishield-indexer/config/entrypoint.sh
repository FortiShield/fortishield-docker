...
export USER=fortishield-indexer
...
if [[ -f bin/opensearch-users ]]; then
  ...
  if [[ -n "$INDEXER_PASSWORD" ]]; then
    [[ -f /usr/share/fortishield-indexer/opensearch.keystore ]] || (run_as_other_user_if_needed opensearch-keystore create)
...
#if [[ "$DISCOVERY" == "single-node" ]] && [[ ! -f "/var/lib/fortishield-indexer/.flag" ]]; then
  # run securityadmin.sh for single node with CACERT, CERT and KEY parameter
#  nohup /securityadmin.sh &
#  touch "/var/lib/fortishield-indexer/.flag"
#fi
